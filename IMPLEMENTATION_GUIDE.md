...
**Purpose:** Fetch and calculate defensive context scores for all teams.

**Key Functions:**
```python
def fetch_team_defense(season, season_type='Regular Season'):
    """
    Fetch team defensive stats.
    Returns: DataFrame with team defensive metrics
    """
    pass

def calculate_defensive_context_score(def_rating, opp_efg, opp_ft_rate):
    """
...
def apply_dynamic_adjustment(regular_df, playoff_df):
    """
    If playoff games >= 10, use playoff DCS.
    Else, use regular season DCS.
    """
    pass
```
...
### 1.3 Create Playoff Play-by-Play Collector

**File:** `src/nba_data/scripts/collect_playoff_playbyplay.py`

**Purpose:** Fetch playoff play-by-play data and apply garbage time filter.

**Key Functions:**
```python
def get_playoff_game_ids(season):
    """
    Get list of all playoff game IDs for a season.
    """
    pass
...
def is_garbage_time(period, time_remaining, score_margin):
    ...
    return (total_seconds <= 300) and (abs(score_margin) >= 15)

def aggregate_to_player_stats(pbp_df):
    """
    Aggregate play-by-play to player-game statistics.
    Calculate TS%, PPG, AST%, etc.
    """
    pass
```
...
**Run:**
```bash
python src/nba_data/scripts/collect_playoff_playbyplay.py --seasons 2023-24
```

**Expected Output:**
- `data/playoff_playbyplay_2023-24_clean.csv` (~200 player-series)

**Note:** This script will take 2-3 hours due to API rate limiting. Progress bar recommended.
...
**Purpose:** Combine all data sources into single training dataset.

**Key Functions:**
```python
def load_all_data(seasons):
    """
    Load regular season, defensive context, and playoff data for all seasons.
    """
    pass

def merge_datasets(regular_df, defense_df, playoff_df, series_df):
    """
    Join all datasets on appropriate keys.
    """
    # Join regular season with playoff stats
    merged = regular_df.merge(
        playoff_df, 
        on='PLAYER_ID',
        suffixes=('_RS', '_PO')
    )
    
    # Add defensive context for opponents
    merged = merged.merge(
        defense_df,
...
def validate_data_quality(df):
    ...
    - No missing values
    - Valid ranges (TS% 0-1, DCS 0-100)
    - Minimum playoff games
    - No extreme outliers (>5 SD)
    """
    pass
```
...
**Expected Output:**
- `data/training_dataset.csv` (~800-1000 rows)

#!/usr/bin/env python3
"""
Script to populate player playtype statistics from NBA Synergy API.

This script fetches detailed play type breakdowns (Isolation, Pick & Roll, etc.)
...
class PlaytypeDataPopulator:
    """Populates player playtype statistics into the database."""
    ...
    def populate_season_playtype_data(
        self,
        ...
        """
        Populate playtype data for an entire season.
        ...
        logger.info(f"Starting playtype data population for {season_year} {season_type}")

        # Get data for all play types
        logger.info("Fetching data for all play types...")
        ...
        for play_type, response_data in all_playtype_data.items():
            try:
                logger.info(f"Processing {play_type} play type...")

                # Parse the response
                records = self.client.parse_playtype_response(response_data)

                if not records:
                    logger.warning(f"No records found for {play_type}")
                    continue
                ...
                logger.info(f"Completed {play_type}: {len(records)} records")

            except Exception as e:
                error_msg = f"Failed to process {play_type}: {e}"
                logger.error(error_msg)
                results["errors"].append(error_msg)

        logger.info(f"Playtype data population complete: {results['records_inserted']} inserted, {results['records_updated']} updated")
        return results
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            for record in records:
                try:
                    ...
                    error_msg = f"Failed to store record for player {record.get('player_id', 'unknown')}: {e}"
                    logger.error(error_msg)
                    results["errors"].append(error_msg)
    ...
    def validate_playtype_data(self, season_type: str = "Regular Season") -> Dict[str, Any]:
        """Validate the quality of stored playtype data."""
        validation_results = {
            "total_records": 0,
            "unique_players": 0,
            "unique_play_types": 0,
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Populate NBA player playtype statistics')
    ...
    if args.validate_only:
        print("🔍 Validating existing playtype data...")
        validation = populator.validate_playtype_data(args.season_type)
    ...
    print(f"🏀 Populating {args.season} {args.season_type} playtype data...")
    ...
    print("\n✅ Playtype data population complete!")
